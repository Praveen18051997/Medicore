...
export default function Sidebar({ isOpen, onClose, collapsed, onToggleCollapse }) {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      ...
      <button
        ...
        className={`w-full flex items-center gap-3 px-4 h-16 border-b border-surface-200/80 dark:border-surface-800/70 text-left hover:bg-surface-100/50 dark:hover:bg-surface-800/40 transition-colors group cursor-pointer ${collapsed ? 'justify-center' : ''}`}
        ...
        {navItems.map((item) => (
          ...
          >
            ...
            {!collapsed && item.label === 'Notifications' && unreadCount > 0 && (
              <span className="px-2 py-0.5 text-[11px] font-bold bg-danger-500 text-white rounded-full">
                {unreadCount}
              </span>
            )}
          </NavLink>
      ...
      <div className="p-3 border-t border-surface-200/80 dark:border-surface-800/70">
        <button
          onClick={handleLogout}
          className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl font-semibold text-sm text-danger-600 dark:text-danger-400 hover:bg-danger-500/10 transition-colors ${
  ...
  return (
    <>
      {/* Mobile overlay */}
      {isOpen && (
        <div className="fixed inset-0 bg-surface-950/60 backdrop-blur-md z-40 lg:hidden" onClick={onClose} />
      )}
